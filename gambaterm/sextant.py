...
requiring 80x48 terminal cells for a 160x144 Game Boy display instead of
160x72 with half-blocks.
"""
...
def _rgb_to_hsv(packed_rgb: int) -> tuple[float, float, float]:
    """Convert packed 0xRRGGBB to HSV tuple."""
...
def _hsv_distance(rgb_a: int, rgb_b: int) -> float:
    """Perceptual distance between two packed RGB colors in HSV space."""
    h1, s1, v1 = _rgb_to_hsv(rgb_a)
    h2, s2, v2 = _rgb_to_hsv(rgb_b)
    dh = min(abs(h1 - h2), 1.0 - abs(h1 - h2))
    return dh * dh + (s1 - s2) ** 2 + (v1 - v2) ** 2


# ---------------------------------------------------------------------------
# Color quantization — ported from termblit.pyx for visual consistency
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Cursor movement — matches termblit.pyx patterns
# ---------------------------------------------------------------------------
...
def _select_bitonal_pair(pixels: list[int]) -> tuple[int, int, int]:
    ...
    if len(unique) == 2:
        ...
        max_dist = max(_hsv_distance(a, b) for a, b in combinations(unique, 2))
        if max_dist == 0:
            max_dist = 1.0
        best_score = -1.0
        ...
        for a, b in combinations(unique, 2):
            ...
            score = 0.85 * (freq / 6) + 0.15 * (_hsv_distance(a, b) / max_dist)
            if score > best_score:
                best_score = score
                c0, c1 = a, b
    ...
    for bit, p in enumerate(pixels):
        if p == c1:
            ...
            if _hsv_distance(p, c1) < _hsv_distance(p, c0):
                index |= (1 << bit)
                fg_count += 1
